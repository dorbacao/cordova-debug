Ext.define('WmsMobile.controller.Recebimento', {
    ...
    recebimentoLancarView_onLoad: function (view) {
        ...
        var lote = cm.getCtrl('loteTextField');
        var serie = cm.getCtrl('serieTextField');
        ...
        if (!itemDocumentoModel.get('SolicitarDataValidade')) {
            vencimento.hide();
            var items = LancamentoRecebimentoModel.getValidations().items;
            if (items[items.length - 1].field == "Vencimento") {
                items.pop();
            }
        } else {
            vencimento.show();
            LancamentoRecebimentoModel.setValidations({ type: 'presence', field: 'Vencimento', message: 'O campo vencimento é obrigatório' });
        }

        if (!itemDocumentoModel.get('SolicitarLote')) {
            lote.hide();
            var items = LancamentoRecebimentoModel.getValidations().items;
            if (items[items.length - 1].field == "Lote") items.pop();
        }
        else
        {
            lote.show();
            LancamentoRecebimentoModel.setValidations({
                type: 'presence', field: 'Lote', message: 'O campo lote é obrigatório'
            });
        }

        if (!itemDocumentoModel.get('SolicitarNumeroSerie')) {
            serie.hide();
            var items = LancamentoRecebimentoModel.getValidations().items;
            if (items[items.length - 1].field == "Serie") items.pop();
        }
        else
        {
            serie.show();
            LancamentoRecebimentoModel.setValidations({
                type: 'presence', field: 'Serie', message: 'O campo série é obrigatório'
            });
        }
    ...
    recebimentoLancarView_botaoConfirmar_onConfirmarRecebimentoLancarView: function (button, e) {
        ...
        var falha = function (records, operation, response) {
            ...
            self.trataResponse(response, function () {
                var message = Mensagem.getErro(response.responseText);
                cm.showMessage('messageTitlebar', message);
                //Ext.Msg.alert("Erro no Lançamento", message.text);
            });
        };
